...
public partial class GitlabSettingsUserControl : GitExtensionsControl, IBuildServerSettingsUserControl
{
    ...
    {
        InitializeComponent();
        GetProjectIdLink.Click += GetProjectIdLink_LinkClicked;
        TokenManagementLink.Click += TokenManagementLink_LinkClicked;
        InstanceUrlTextBox.PropertyChanged += InstanceUrlTextBox_TextChanged;
        InitializeComplete();
    }
    ...
    private async Task<int?> UpdateProjectIdAsync(string? host, string? apiToken)
    {
        ...
        try
        {
            using GitlabApiClient apiClient = new(host, apiToken ?? string.Empty);
    ...
    private void GetProjectIdLink_LinkClicked(object? sender, EventArgs e)
    {
        ...
        ThreadHelper.FileAndForget(async () =>
        {
            ...
            {
                GetProjectIdStatusText.IsVisible = true;
            }
        });
    }

    private void TokenManagementLink_LinkClicked(object? sender, EventArgs e)
    {
    ...
    private void InstanceUrlTextBox_TextChanged(object? sender, AvaloniaPropertyChangedEventArgs e)
    {
        ...
        bool isValidUrl = Uri.IsWellFormedUriString(InstanceUrlTextBox.Text, UriKind.Absolute);
        TokenManagementLink.IsEnabled = isValidUrl;
        GetProjectIdLink.IsEnabled = isValidUrl;
    }
}
