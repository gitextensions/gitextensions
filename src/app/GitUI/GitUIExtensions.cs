...
public static partial class GitUIExtensions
{
        ...
        bool forceFileView = false)
    {
        ...
        {
            GitSubmoduleStatus? status;
            if (item.Item.GetSubmoduleStatusAsync() is Task<GitSubmoduleStatus?> task && task is not null)
            {
            ...
            {
                status = await SubmoduleHelpers.GetSubmoduleDiffChangesAsync(fileViewer.Module, item.Item.Name, item.Item.OldName, firstId, item.SecondRevision.ObjectId, cancellationToken);
            }
