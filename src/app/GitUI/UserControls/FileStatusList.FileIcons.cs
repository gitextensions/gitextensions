...
partial class FileStatusList
{
    private void LoadFileIcons(IEnumerable<TreeNode> nodes, CancellationToken cancellationToken)
    {
        ...
        if (missingIconsByExtension.Count > 0 && Module?.WorkingDir is string workingDir && missingIconsByExtension.Values.First().Nodes[0].TreeView is TreeView treeView)
        {
            ThreadHelper.FileAndForget(() => LoadFileIconsAsync(missingIconsByExtension, fileName => _iconProvider.Get(workingDir, fileName), _imageListData.ImageList.ImageSize, treeView, cancellationToken));
        }
