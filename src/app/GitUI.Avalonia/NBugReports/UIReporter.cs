...
using GitUI.Compat;
using Form = Avalonia.Controls.Window;
...
///  and optionally launching the native Avalonia report form.
/// </summary>
internal sealed class UIReporter : IBugReporter
{
    ...
    private static Form? OwnerForm
        => (Application.Current?.ApplicationLifetime as IClassicDesktopStyleApplicationLifetime)?.Windows.FirstOrDefault(window => window.IsActive)
           ?? (Application.Current?.ApplicationLifetime as IClassicDesktopStyleApplicationLifetime)?.MainWindow;

    private static IntPtr OwnerFormHandle
        => (OwnerForm as WinFormsShims.IWin32Window)?.Handle ?? IntPtr.Zero;
...
#pragma warning restore S1075
    ...
    private static TaskDialogPage CreateErrorReport(Exception exception, string rootError, StringBuilder text, OperationInfo operationInfo)
    {
        ...
        {
            ShowNBug(OwnerForm as WinFormsShims.IWin32Window, exception, operationInfo.IsExternalOperation, operationInfo.IsUserExternalOperation, operationInfo.IsTerminating);
        };
    ...
    private static TaskDialogPage CreateFailedToLoadAnAssemblyReport(Exception exception, bool isTerminating)
    {
        ...
        if (!IsDotNetFrameworkAssembly(fileName) && !IsVCRuntimeDll(fileName))
        {
            ...
            reportButton.Click += (_, _) => ShowNBug(OwnerForm as WinFormsShims.IWin32Window, exception, isExternalOperation: false, isUserExternalOperation: false, isTerminating);
    ...
    private static void ShowGitRepo(Form? ownerForm, string? workingDir)
    {
        ...
        {
            Dispatcher.UIThread.Post(() => formBrowse.SetWorkingDir(workingDir));
        }
    }

    private static void ShowNBug(WinFormsShims.IWin32Window? owner, Exception exception, bool isExternalOperation, bool isUserExternalOperation, bool isTerminating)
    {
        WinFormsShims.DialogResult result = BugReportLauncher.Show(
            owner as Window,
            exception,
    ...
    internal readonly struct TestAccessor
    {
        ...
        internal static IBugReportLauncher BugReportLauncher
        {
