...
public sealed class AzureDevopsExternalLinkDefinitionExtractor : ExternalLinkDefinitionExtractor
{
    ...
    public override IImage Icon => Images.VisualStudioTeamServices;
    ...
    public override IList<ExternalLinkDefinition> GetDefinitions(string remoteUrl)
    {
        ...
        {
            _azureDevOpsRemoteParser.TryExtractAzureDevopsDataFromRemoteUrl(
                remoteUrl,
                out accountName,
                out _,
                out repoName);
        ...
        ExternalLinkDefinition definition = new()
        {
            ...
            LinkFormats =
            {
                ...
                {
                    Caption = string.Format(ViewCommitLink.Text, ServiceName),
                    Format = $"{azureDevopsUrl}/_git/{repoName}/commit/%COMMIT_HASH%",
                },
                new ExternalLinkFormat
                {
                    Caption = string.Format(ViewProjectLink.Text, ServiceName),
                    Format = $"{azureDevopsUrl}/{repoName}",
                },
            },
        };
        ...
        externalLinkDefinitions.Add(new ExternalLinkDefinition
        {
            ...
            LinkFormats =
            {
                ...
                {
                    Caption = "#{0}",
                    Format = $"{azureDevopsUrl}/{repoName}/_workitems/edit/{{0}}",
                },
            },
        });
