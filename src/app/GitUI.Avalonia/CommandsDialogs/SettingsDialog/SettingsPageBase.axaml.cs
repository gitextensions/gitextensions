...
public abstract partial class SettingsPageBase : TranslatedControl, ISettingsPage
{
    ...
    private readonly List<PluginSettingBinding> _avaloniaBindings = [];
    ...
    protected ToolTip ToolTip { get; } = new();
    ...
    public static T Create<[MeansImplicitUse] T>(ISettingsPageHost pageHost, IServiceProvider serviceProvider) where T : SettingsPageBase
    {
        ...
        // Avalonia constraint: DPI, remove-word and visual-style behavior is native to the control framework.
    ...
    protected virtual void SettingsToPage()
    {
        ...
        foreach (PluginSettingBinding binding in _avaloniaBindings)
        {
    ...
    protected void AddSettingBinding(ISetting<string> setting, ComboBox comboBox)
    {
        ...
        _avaloniaBindings.Add(SettingControlBindingsProvider.CreateControlBinding(adapter, comboBox));
    }
    ...
    private static IReadOnlyList<string> GetChildrenText(Control control)
    {
        ...
        while (queue.Count != 0)
        {
            ...
            if (!next.IsVisible || next is NumericUpDown)
            {
                // skip: invisible; some input controls
                continue;
            }

            string? text = next switch
            {
                TextBlock textBlock => textBlock.Text,
                TextBox textBox => textBox.Text,
                ComboBox comboBox => comboBox.Text ?? comboBox.SelectedItem?.ToString(),
                ContentControl { Content: string content } => content,
                HeaderedContentControl { Header: string header } => header,
                _ => null,
            };

            if (next.IsEnabled && !string.IsNullOrWhiteSpace(text))
            {
            ...
            foreach (Control child in next.GetLogicalChildren().OfType<Control>())
            {
                queue.Enqueue(child);
            }
        }
...
public class StringComboBoxAdapter : ChoiceSetting
{
    ...
    {
        // Avalonia constraint: the native control is supplied directly to the native binding factory.
    }
}
