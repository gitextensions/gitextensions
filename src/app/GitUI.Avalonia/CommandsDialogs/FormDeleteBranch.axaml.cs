...
public sealed partial class FormDeleteBranch : GitExtensionsDialog
{
    ...
    private readonly IReadOnlyList<string> _defaultBranches = [];
    ...
    protected override void OnRuntimeLoad(EventArgs e)
    {
        ...
        if (_defaultBranches.Count > 0)
        {
            Branches.SetSelectedText(string.Join(' ', _defaultBranches));
        }
    ...
    private void Delete_Click(object? sender, EventArgs e)
    {
        ...
        if (_currentBranch is not null && selectedBranches.Any(branch => branch.Name == _currentBranch))
        {
            MessageBoxes.Show(
                this,
                string.Format(_cannotDeleteCurrentBranchMessage.Text, _currentBranch),
        ...
        IGitCommand command = Commands.DeleteBranch(selectedBranches, force: true);
        if (UICommands.StartCommandLineProcessDialog(this, command))
        {
            Close();
        }
    }
    ...
    private IGitRef[] HandleWorktreeBranches(IGitRef[] selectedBranches)
    {
        ...
        if (classification.LinkedWorktreeBranches.Count > 0)
        {
            string branchList = string.Join(
                "\n",
                classification.LinkedWorktreeBranches.Select(item => $"\u2022 {item.Branch.Name}  \u2192  {item.Worktree.Path}"));
            TaskDialogPage page = new()
            {
        ...
        return excludedBranches.Count > 0
            ? selectedBranches.Where(branch => !excludedBranches.Contains(branch.Name)).ToArray()
            : selectedBranches;
    }
        ...
        string currentWorkingDir)
    {
        ...
        for (int i = 0; i < worktrees.Count; i++)
        {
            ...
            {
                if (selectedBranches.Any(branch => branch.Name == worktree.Branch))
                {
