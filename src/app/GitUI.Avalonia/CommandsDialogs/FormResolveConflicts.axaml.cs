...
public partial class FormResolveConflicts : GitModuleForm
{
        ...
        : base(commands, true)
    {
        ...
        // The single visible WinForms grid column becomes the list item template
        // (the hidden Author column had no data source upstream: see its TODO there).
        ConflictedFiles.ItemTemplate = new FuncDataTemplate<object>(
            (item, _) => new TextBlock { Text = item is ConflictData conflict ? conflict.Filename : string.Empty },
            supportsRecycling: false);

        ConflictedFiles.AddHandler(PointerPressedEvent, ConflictedFiles_CellMouseDown, RoutingStrategies.Tunnel);
        ConflictedFiles.SelectionChanged += ConflictedFiles_SelectionChanged;
        ConflictedFiles.DoubleTapped += ConflictedFiles_DoubleClick;
    ...
    private void Initialize()
    {
        using (WaitCursorScope.Enter())
        {
            ...
            if (ConflictedFiles.SelectedIndex >= 0)
            {
                oldSelectedRow = ConflictedFiles.SelectedIndex;
                isLastRow = ConflictedFiles.ItemCount - 1 == oldSelectedRow;
            }

            List<ConflictData> conflictDataList = [.. ThreadHelper.JoinableTaskFactory.Run(() => Module.GetConflictsAsync())];
            ConflictedFiles.ItemsSource = conflictDataList;

            // if the last row was previously selected, select the last row again
            if (isLastRow && oldSelectedRow >= ConflictedFiles.ItemCount)
            {
            ...
            if (ConflictedFiles.ItemCount > oldSelectedRow)
            {
                ...
                // row (WinForms got the first row for free from the data binding).
                ConflictedFiles.SelectedIndex = oldSelectedRow;
                ConflictedFiles.ScrollIntoView(oldSelectedRow);
            }

            // Update UI after tool configuration is known
            UpdateConflictedFilesMenu();

            OpenMergetool.Header = _openMergeToolItemText.Text + " " + _mergetool;
            openMergeToolBtn.Content = _button1Text.Text + " " + _mergetool;

            if (_inTheMiddleOfRebase)
            {
                ContextChooseLocal.Header = _contextChooseLocalRebaseText.Text;
                ToolTip.SetTip(ContextChooseLocal, _changesTakeOnlyLocalRebaseTooltip.Text);
                labelLocalCurrent.Text = labelLocalCurrent.Text!.UpdateSuffixWithinParenthesis(_theirs.Text);
                ToolTip.SetTip(labelLocalCurrent, _changesLocalRebaseTooltip.Text);

                ContextChooseRemote.Header = _contextChooseRemoteRebaseText.Text;
                ToolTip.SetTip(ContextChooseRemote, _changesTakeOnlyRemoteRebaseTooltip.Text);
                labelRemoteIncoming.Text = labelRemoteIncoming.Text!.UpdateSuffixWithinParenthesis(_ours.Text);
                ToolTip.SetTip(labelRemoteIncoming, _changesRemoteRebaseTooltip.Text);
            }
            else
            {
                ContextChooseLocal.Header = _contextChooseLocalMergeText.Text;
                ToolTip.SetTip(ContextChooseLocal, _changesTakeOnlyLocalMergeTooltip.Text);
                labelLocalCurrent.Text = labelLocalCurrent.Text!.UpdateSuffixWithinParenthesis(_ours.Text);
                ToolTip.SetTip(labelLocalCurrent, _changesLocalMergeTooltip.Text);

                ContextChooseRemote.Header = _contextChooseRemoteMergeText.Text;
                ToolTip.SetTip(ContextChooseRemote, _changesTakeOnlyRemoteMergeTooltip.Text);
                labelRemoteIncoming.Text = labelRemoteIncoming.Text!.UpdateSuffixWithinParenthesis(_theirs.Text);
                ToolTip.SetTip(labelRemoteIncoming, _changesRemoteMergeTooltip.Text);
            }

            ToolTip.SetTip(ContextChooseBase, _contextChooseBaseTooltip.Text);
    ...
    {
        return (ConflictData)ConflictedFiles.SelectedItems![0]!;
    }
    ...
    {
        IReadOnlyList<ConflictData> conflicts = ConflictedFiles.SelectedItems!
            .Cast<ConflictData>()
            .ToArray();
    ...
    private bool TryMergeWithScript(string fileName, string? baseFileName, string? localFileName, string? remoteFileName)
    {
        ...
        try
        {
            ...
            string dir = Path.Join(Path.GetDirectoryName(WinFormsShims.Application.ExecutablePath)!, "Diff-Scripts").EnsureTrailingPathSeparator();
            if (Directory.Exists(dir))
            {
                    ...
                    File.Exists(Path.Join(dir!, mergeScript)))
                {
                    if (MessageBoxes.Show(this, string.Format(_uskUseCustomMergeScript.Text, mergeScript),
                                        _uskUseCustomMergeScriptCaption.Text, WinFormsShims.MessageBoxButtons.YesNo, WinFormsShims.MessageBoxIcon.Question) ==
        ...
        {
            MessageBoxes.Show(this, "Merge using script failed.\n" + ex, TranslatedStrings.Error, WinFormsShims.MessageBoxButtons.OK, WinFormsShims.MessageBoxIcon.Error);
        }
    ...
    private void UseMergeWithScript(string fileName, string mergeScript, string? baseFileName, string? localFileName, string? remoteFileName)
    {
            ...
            WinFormsShims.MessageBoxButtons.YesNo, WinFormsShims.MessageBoxIcon.Question) == WinFormsShims.DialogResult.Yes)
        {
            ...
            {
                MessageBoxes.Show(this, _fileUnchangedAfterMerge.Text, "Information", WinFormsShims.MessageBoxButtons.OK, WinFormsShims.MessageBoxIcon.Information);
            }
            else
            {
                StageFile(fileName);
                ...
                CheckForRemoteRevision(item))
            {
                ...
                if (FileHelper.IsBinaryFileName(Module, item.Local.Filename))
                {
                    if (MessageBoxes.Show(this, string.Format(_fileIsBinary.Text, _mergetool),
                            TranslatedStrings.Warning, WinFormsShims.MessageBoxButtons.YesNo, WinFormsShims.MessageBoxIcon.Warning,
                ...
                if (item.Base.Filename is null)
                {
                    ...
                    WinFormsShims.DialogResult result = MessageBoxes.Show(this, text, _noBaseFileMergeCaption.Text,
                        WinFormsShims.MessageBoxButtons.YesNoCancel, WinFormsShims.MessageBoxIcon.Question);
                    if (result == WinFormsShims.DialogResult.Yes)
                    {
                        Use2WayMerge(ref arguments);
                    }

                    if (result == WinFormsShims.DialogResult.Cancel)
                    {
                        return;
                    }
                }
                ...
                catch (Exception)
                {
                    ...
                    MessageBoxes.Show(this, text, _noBaseFileMergeCaption.Text,
                        WinFormsShims.MessageBoxButtons.OK, WinFormsShims.MessageBoxIcon.Error);
                    return;
                }
    ...
    private bool InitMergetool()
    {
        ...
        if (string.IsNullOrEmpty(_mergetool))
        {
            MessageBoxes.Show(this, _noMergeTool.Text, TranslatedStrings.Error, WinFormsShims.MessageBoxButtons.OK, WinFormsShims.MessageBoxIcon.Error);
            return false;
        }
        ...
        using (WaitCursorScope.Enter())
        {
            ...
            {
                MessageBoxes.Show(this, _noMergeToolConfigured.Text, TranslatedStrings.Warning, WinFormsShims.MessageBoxButtons.OK, WinFormsShims.MessageBoxIcon.Warning);
                return false;
            }
    ...
    {
        return ConflictedFiles.SelectedItems?.Count == 1;
    }
    ...
    private void SetAvailableCommands(bool enabled)
    {
        ...
        OpenMergetool.IsEnabled = enabled && mergeToolExtrasConfigured;
        customMergetool.IsEnabled = enabled;
        openMergeToolBtn.IsEnabled = enabled && mergeToolExtrasConfigured;
        ContextOpenLocalWith.IsEnabled = enabled;
        ContextOpenRemoteWith.IsEnabled = enabled;
        ContextOpenBaseWith.IsEnabled = enabled;
        ContextSaveLocalAs.IsEnabled = enabled;
        ContextSaveRemoteAs.IsEnabled = enabled;
        ContextSaveBaseAs.IsEnabled = enabled;
        openToolStripMenuItem.IsEnabled = enabled;
        openWithToolStripMenuItem.IsEnabled = enabled;
        openFolderToolStripMenuItem.IsEnabled = enabled;
        fileHistoryToolStripMenuItem.IsEnabled = enabled;
    }
    ...
    private void ChooseBaseOnConflict(string fileName)
    {
        ...
        {
            MessageBoxes.Show(this, _chooseBaseFileFailedText.Text, TranslatedStrings.Error, WinFormsShims.MessageBoxButtons.OK, WinFormsShims.MessageBoxIcon.Error);
        }
    }
    ...
    private void ChooseLocalOnConflict(string fileName)
    {
        ...
        {
            MessageBoxes.Show(this, _chooseLocalFileFailedText.Text, TranslatedStrings.Error, WinFormsShims.MessageBoxButtons.OK, WinFormsShims.MessageBoxIcon.Error);
        }
    }
    ...
    private void ChooseRemoteOnConflict(string fileName)
    {
        ...
        {
            MessageBoxes.Show(this, _chooseRemoteFileFailedText.Text, TranslatedStrings.Error, WinFormsShims.MessageBoxButtons.OK, WinFormsShims.MessageBoxIcon.Error);
        }
    }
    ...
    private void OpenSideWith(string side)
    {
        using (WaitCursorScope.Enter())
        {
            ...
            {
                MessageBoxes.Show(this, _failureWhileOpenFile.Text, TranslatedStrings.Error, WinFormsShims.MessageBoxButtons.OK, WinFormsShims.MessageBoxIcon.Error);
            }
    ...
    private void ConflictedFiles_CellMouseDown(object? sender, PointerPressedEventArgs e)
    {
        if (e.GetCurrentPoint(ConflictedFiles).Properties.IsRightButtonPressed)
        {
            if (HasMultipleRowsSelected())
            {
            ...
            // Select the row under the pointer (the WinForms grid does this itself).
            ListBoxItem? row = (e.Source as Avalonia.Visual)?.FindAncestorOfType<ListBoxItem>(includeSelf: true);
            int lastRow = row is null ? -1 : ConflictedFiles.IndexFromContainer(row);
            ConflictedFiles.UnselectAll();
            if (lastRow >= 0 && ConflictedFiles.ItemCount > lastRow)
            {
                ConflictedFiles.SelectedIndex = lastRow;
            }
    ...
    private void SaveAs(string side)
    {
        using (WaitCursorScope.Enter())
        {
            ...
            // The WinForms SaveFileDialog becomes the Avalonia storage provider
            ...
            string? savePath = DispatcherPump.Wait(async () =>
            {
                IStorageFile? file = await StorageProvider.SaveFilePickerAsync(new FilePickerSaveOptions
                {
                    ...
                    FileTypeChoices =
                    [
                        new FilePickerFileType(string.Format(_currentFormatFilter.Text, ext.TrimStart('.'))) { Patterns = [$"*{ext}"] },
                        new FilePickerFileType(_allFilesFilter.Text) { Patterns = ["*.*"] },
                    ],
                });
            ...
            {
                if (!Module.HandleConflictsSaveSide(conflictData.Filename, savePath, side))
                {
                    MessageBoxes.Show(this, _failureWhileSaveFile.Text, TranslatedStrings.Error, WinFormsShims.MessageBoxButtons.OK, WinFormsShims.MessageBoxIcon.Error);
                }
            }
        }
    }
    ...
    private void ConflictedFilesContextMenu_Opening(object sender, System.ComponentModel.CancelEventArgs e)
    {
        if (ConflictedFiles.SelectedItems is not { Count: > 0 })
        {
            e.Cancel = true;
            return;
        }
        ...
        ConflictedFilesContextMenu.IsEnabled = !string.IsNullOrEmpty(fileName);

        if (ConflictedFilesContextMenu.IsEnabled)
        {
            if (HasMultipleRowsSelected())
            {
                SetAvailableCommands(false);
    ...
    private void DisableInvalidEntriesInConflictedFilesContextMenu(string fileName)
    {
        ...
        {
            ContextSaveLocalAs.IsEnabled = false;
            ContextOpenLocalWith.IsEnabled = false;
        }

        if (string.IsNullOrEmpty(conflictedFileNames.Remote.Filename))
        {
            ContextSaveRemoteAs.IsEnabled = false;
            ContextOpenRemoteWith.IsEnabled = false;
        }

        if (string.IsNullOrEmpty(conflictedFileNames.Base.Filename))
        {
            ContextSaveBaseAs.IsEnabled = false;
            ContextOpenBaseWith.IsEnabled = false;
        }
    }
    ...
    private static void OpenAs(string filePath)
    {
        if (OperatingSystem.IsWindows())
        {
        ...
        {
            // There is no generic "open with" chooser outside Windows;
    ...
    private void openFolderToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        if (filePath is not null)
        {
            ...
            else if (Path.GetDirectoryName(filePath) is { } directory)
            {
                // Selecting a file needs Windows File Explorer; open the folder instead.
    ...
    {
        if (KeysMapper.ToKeys(e) == WinFormsShims.Keys.Enter)
        {
            OpenMergeTool();
            e.Handled = true;
        }
    ...
    {
        // The WinForms context menu ShortcutKeys work while the menu is closed.
        WinFormsShims.Keys keyData = KeysMapper.ToKeys(e);
        if (keyData == (WinFormsShims.Keys.Control | WinFormsShims.Keys.D1))
        {
            ContextChooseLocal_Click(this, EventArgs.Empty);
            e.Handled = true;
            return;
        }

        if (keyData == (WinFormsShims.Keys.Control | WinFormsShims.Keys.D2))
        {
            ContextChooseRemote_Click(this, EventArgs.Empty);
            e.Handled = true;
            return;
        }

        if (keyData == (WinFormsShims.Keys.Control | WinFormsShims.Keys.D3))
        {
            ContextChooseBase_Click(this, EventArgs.Empty);
            e.Handled = true;
            return;
        }
