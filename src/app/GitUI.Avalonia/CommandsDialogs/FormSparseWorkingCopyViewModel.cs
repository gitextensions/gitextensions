...
public class FormSparseWorkingCopyViewModel : INotifyPropertyChanged
{
    ...
    public void RefreshWorkingCopy()
    {
        ...
        // Cross-platform constraint: WinForms Form.ActiveForm has no Avalonia equivalent; resolve the active desktop window as the modal owner.
        WinFormsShims.IWin32Window? owner = null;
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            owner = desktop.Windows.FirstOrDefault(window => window.IsActive) as WinFormsShims.IWin32Window
                ?? desktop.MainWindow as WinFormsShims.IWin32Window;
        }
