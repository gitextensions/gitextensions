...
public class GitVersion : IComparable<GitVersion>, IGitVersion
{
    ...
    {
        if (_current.TryGetValue(gitIdentifiable, out GitVersion? current) && !current.IsUnknown)
        {
            return current;
        }
        ...
        try
        {
            ...
            current
                = _current[gitIdentifiable]
                = new GitVersion(output);
            if (current < LastVersionWithoutKnownLimitations)
            {
                // Report the last supported version rather than the last version without known issues
                MessageBox.Show(null, $"{current} is lower than {LastSupportedVersion}. Some commands can fail.", "Unsupported Git version", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
