...
{
    public class GitRemoteController
    {
        private static readonly object SyncRoot = new object();
        private readonly GitModule _module;


        public GitRemoteController(GitModule module)
        {
            _module = module;
        ...
        /// <summary>
        /// Gets the list of remotes configured in .git/config.
        /// </summary>
        ...
        /// <summary>
        ///   Reorders the remotes according to the specified direction.
        /// </summary>
        /// <param name="remote">The remote to move up or down.</param>
        /// <param name="direction">
        ///   <para>The direction to move the remote.</para>
        ///   <para>Only <see cref="ArrowDirection.Up"/> and <see cref="ArrowDirection.Down"/> are supported.</para>
        /// </param>
        public void MoveRemote(GitRemote remote, ArrowDirection direction)
        {
            if (remote == null)
            {
                throw new ArgumentNullException("remote");
            ...
            if (direction == ArrowDirection.Up && remote == Remotes.First())
            {
                return;
            }
            if (direction == ArrowDirection.Down && remote == Remotes.Last())
            {
                return;
            }
            ...
            {
                Remotes.Insert(remote.Order - 1, remote);
            }
        ...
        /// <summary>
        /// Reloads the remotes from the .git/config.
        /// </summary>
        public void Refresh()
        {
            LoadRemotes();
        ...
        /// <summary>
        ///   Saves the remote details by creating a new or updating an existing remote entry.
        /// </summary>
        ...
        public GitRemoteSaveResult SaveRemote(GitRemote remote, string remoteName, string remoteUrl, string remotePushUrl, string remotePuttySshKey)
        {
            ...
            {
                if (!remote.Name.Equals(remoteName, StringComparison.OrdinalIgnoreCase))
                {
                    // the name of the remote changed - perform rename
                    output = _module.RenameRemote(remote.Name, remoteName);
                }

                if (!remote.Url.Equals(remoteUrl, StringComparison.OrdinalIgnoreCase))
                {
        ...
        private void LoadRemotes()
        {
            ...
            lock (SyncRoot)
            {
                ...
                if (gitRemotes.Any())
                {
                    ...
                    {
                        int order;
                        var orderValue = _module.GetPathSetting(string.Format(SettingKeyString.RemoteGitExOrder, remote));
                        ...
                        {
                            Name = remote,
                            Url = _module.GetPathSetting(string.Format(SettingKeyString.RemoteUrl, remote)),
                            Push = _module.GetSettings(string.Format(SettingKeyString.RemotePush, remote)).ToList(),
                            PushUrl = _module.GetPathSetting(string.Format(SettingKeyString.RemotePushUrl, remote)),
                            PuttySshKey = _module.GetPathSetting(string.Format(SettingKeyString.RemotePuttySshKey, remote)),
                            Order = order,
                        });
                    }

                    Remotes.AddAll(remotes.OrderBy(x => x.Order).ThenBy(x => x.Name));
                    // reset the order to ensure consistency and safety in case the values were manually changed
                    // also if the remote was added by "git remote" command outside GitExt it won't have the order set
                    ResyncOrderValues();
                }
            }
        }

        private void ResyncOrderValues()
        {
            int i = 0;
            Remotes.ForEach(x => x.Order = i++);
            // persist the state
            UpdateRemotesOrder();
        }

        private void UpdateRemotesOrder()
        {
            Remotes.ForEach(remote =>
            {
                _module.SetPathSetting(string.Format(SettingKeyString.RemoteGitExOrder, remote.Name), remote.Order.ToString());
            });
        }
        ...
        private void UpdateSettings(string settingName, string value)
        {
            ...
            {
                _module.SetPathSetting(settingName, value);
            }
            else
            {
