...
namespace GitCommandsTests.Remote
{
    ...
    internal class ConfigFileRemoteSettingsManagerTests
    {
        ...
        {
            ((Action)(() => _remotesManager.SaveRemote(null, null, "b", "c", "d", "f"))).Should().Throw<ArgumentNullException>()
                .WithMessage("Value cannot be null. (Parameter 'remoteName')");
            ((Action)(() => _remotesManager.SaveRemote(null, "", "b", "c", "d", "f"))).Should().Throw<ArgumentNullException>()
                .WithMessage("Value cannot be null. (Parameter 'remoteName')");
            ((Action)(() => _remotesManager.SaveRemote(null, "  ", "b", "c", "d", "f"))).Should().Throw<ArgumentNullException>()
                .WithMessage("Value cannot be null. (Parameter 'remoteName')");
        }
