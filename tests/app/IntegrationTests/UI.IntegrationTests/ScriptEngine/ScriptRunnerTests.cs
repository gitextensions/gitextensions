...
public class ScriptRunnerTests
{
    ...
    private IUserScriptsStorage _userScriptsStorage;

    [SetUp]
    public void Setup()
    {
        DistributedScriptsManager scriptsManager = InitScriptManager();
        ...
        DistributedScriptsManager InitScriptManager()
        {
            _userScriptsStorage = Substitute.For<IUserScriptsStorage>();
            _userScriptsStorage.Load(Arg.Any<DistributedSettings>()).Returns(x =>
                new List<ScriptInfo>()
                {
                new()
                {
            ...
            GitModuleTestHelper testHelper = new();
            string userRoamingConfigFilePath = testHelper.CreateRepoFile("GitExtensions.settings", fileContent: "<dictionary/>");
            DistributedSettings userRoaming = new(lowerPriority: null, new GitExtSettingsCache(userRoamingConfigFilePath), SettingLevel.Global);

            DistributedScriptsManager scriptsManager = InitScriptManager();
            scriptsManager.Initialize(userRoaming);
    ...
    public void RunScript_without_command_shall_return_false([Values(null, "")] string command)
    {
        ...
        bool result = ScriptRunner.RunScript(_exampleScript, _mockForm, _mockForm.UICommands);
