...
namespace CreateLocalBranches
{
    public partial class CreateLocalBranchesForm : Form
    {
        ...
        {
            string[] references = m_gitUiCommands.Module.RunGit("branch -a")
            ...
            foreach (string reference in references)
            {
                try
                {
                    ...
                    if (branchName.StartsWith("remotes/" + Remote.Text + "/"))
                        m_gitUiCommands.Module.RunGit(string.Concat("branch --track ", branchName.Replace("remotes/" + Remote.Text + "/", ""), " ", branchName));
                }
                catch
                {
                }
