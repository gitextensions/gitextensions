// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SettingsOverride.cs" company="NBusy Project">
...
namespace NBug.Properties
{
	...
	public static class SettingsOverride
	{
		...
		/// or by some other library, it shall be done before any of the static members of the <see cref="NBug.Settings"/> class is accessed.
		...
		public static void LoadCustomSettings(Stream settingsFile)
		{
			...
			try
			{
				NBug.Settings.LoadCustomSettings(XElement.Load(settingsFile));
			}
			catch (XmlException)
			{
				// Root element is missing so recreate the configuration file
				NBug.Settings.LoadCustomSettings(XElement.Parse("<?xml version=\"1.0\" encoding=\"utf-8\" ?><configuration></configuration>"));
			}
		}
