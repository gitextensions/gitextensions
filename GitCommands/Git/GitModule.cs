...
namespace GitCommands
{
    ...
    {
        public ConflictData(ConflictedFileData @base, ConflictedFileData local, ConflictedFileData remote)
        {
            Base = @base;
            Local = local;
            Remote = remote;
    ...
    public sealed class GitModule : IGitModule
    {
        ...
        public void RunGitK()
        {
            ...
            {
                var executable = AppSettings.GitCommand
                    .Replace("git.cmd", "gitk.cmd")
                    .Replace("bin\\git.exe", "cmd\\gitk.cmd")
                    .Replace("bin/git.exe", "cmd/gitk.cmd");

                RunExternalCmdDetached(
                    "cmd.exe",
                    "/c \"\"" + executable + "\" --branches --tags --remotes\"");
            }
        }
        ...
        /// <param name="recursiveSubmodules">If '1', check whether all submodule commits used by the revisions to be pushed are available on a remote tracking branch; otherwise, the push will be aborted.</param>
        /// <returns>'git push' command with the specified parameters.</returns>
        public string PushCmd(string remote, string fromBranch, string toBranch, ForcePushOptions force, bool track, int recursiveSubmodules)
        {
            remote = remote.ToPosixPath();
        ...
        private void UpdateIndex(Lazy<SynchronizedProcessReader> processReader, string filename)
        {
            ...
            byte[] bytearr = EncodingHelper.ConvertTo(
                SystemEncoding,
        ...
        public IList<GitItemStatus> GetAllChangedFiles(
            bool excludeIgnoredFiles = true,
            bool excludeAssumeUnchangedFiles = true,
        ...
        public string GetCombinedDiffContent(GitRevision revisionOfMergeCommit, string filePath, string extraArgs, Encoding encoding)
        {
            var cmd = string.Format(
                "diff-tree {4} --no-commit-id {0} {1} {2} -- {3}",
                extraArgs,
